...
class SessionRenderer( object ):
    ...
    def set_s3_conn( self ):
        self.s3_conn =  S3Connection(   self.access_key, 
                                        self.secret_key )
    def render( self, username, session_name ):
        working_path = os.path.join( self.working_dir, 'powershame-rendering', username, session_name )
        video_path=os.path.join( working_path, session_name+'.mkv' )
        try:
            os.makedirs( working_path )
        except OSError:
            pass
        self.download_shots( working_path, username, session_name )
        self.make_video( working_path, video_path )
        return video_path
        self.upload_video( video_path, username, session_name )
        return 
    def download_shots( self, working_path, username, session_name ):
        bucket = self.s3_conn.get_bucket( self.shots_bucket )
        shot_keys_to_download = filter( is_shot, bucket.list( prefix=username+'/'+session_name) )
    ...
    def upload_video( self, video_path, key_path ):
        ...
        url = self.s3_conn.generate_url(3600*48, 'GET', bucket=self.video_bucket, key=key_path)
        return url
